...
# Simple pattern for ip=1.2.3.4 in the log line
IP_REGEX = re.compile(r"ip=(\d+\.\d+\.\d+\.\d+)")


def run(path, matches, max_print, jsonl_out=None, count_only=False):
    """Stream file, apply matcher, optionally print lines and collect IP stats."""
    printed = 0
    total = 0
    ip_counts = Counter()
    ...
    try:
        with open(path, "r", encoding="utf-8", errors="replace") as f:
            for line_num, line in enumerate(f, start=1):
                ...
                # update IP stats from ip=...
                ip_match = IP_REGEX.search(line)
                if ip_match:
                    ip_counts[ip_match.group(1)] += 1

                # print limited, unless in count-only mode
                if not count_only and printed < max_print:
                    print(f"{line_num}: {line}")
                    printed += 1

                # write JSONL record if enabled
                if out_f:
...
def make_matcher(keyword, regex, ignore_case):
    """Return a function(line) -> bool based on keyword/regex/ignore_case."""
...
def build_args():
    """Build an argparse.ArgumentParser for the CLI."""
    ...
    p.add_argument(
        "--count-only",
        action="store_true",
        help="Only print summary counts and top IPs (no matching lines).",
    )
    return p
...
def main():
    ...
    matcher = make_matcher(args.keyword, args.regex, args.ignore_case)
    run(args.path, matcher, args.max, args.jsonl, count_only=args.count_only)
